...
// A usable bounding box: four finite numbers with lat/lon ranges in order.
// When present it confines the watch to a region (e.g. Japan) via the FDSN
// minlat/maxlat/minlon/maxlon parameters, replacing the point-radius scope.
function hasBounds(bounds) {
  if (!bounds) return false
  var minLat = parseNumber(bounds.minLat)
  var maxLat = parseNumber(bounds.maxLat)
  var minLon = parseNumber(bounds.minLon)
  var maxLon = parseNumber(bounds.maxLon)
...
function inBounds(event, bounds) {
  if (!hasBounds(bounds) || !event) return false
  return event.lat >= parseNumber(bounds.minLat) && event.lat <= parseNumber(bounds.maxLat)
    && event.lon >= parseNumber(bounds.minLon) && event.lon <= parseNumber(bounds.maxLon)
}
...
function fdsnUrl(options) {
  ...
  if (hasBounds(options.bounds)) {
    url += "&minlat=" + encodeURIComponent(String(clampNumber(options.bounds.minLat, -90, 90, -90)))
      + "&maxlat=" + encodeURIComponent(String(clampNumber(options.bounds.maxLat, -90, 90, 90)))
      + "&minlon=" + encodeURIComponent(String(clampNumber(options.bounds.minLon, -180, 180, -180)))
      + "&maxlon=" + encodeURIComponent(String(clampNumber(options.bounds.maxLon, -180, 180, 180)))
  }
  return url
}
...
function feedUrls(options) {
  ...
  // A bounding box (e.g. Japan) wins over the point-radius scope; the FDSN
  // API confines the query to the box natively.
  if (hasBounds(options.bounds)) {
    return [fdsnUrl({
      minMagnitude: minMag,
...
function filterEvents(events, options) {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    if (hasBounds(options.bounds) && !inBounds(event, options.bounds)) continue
